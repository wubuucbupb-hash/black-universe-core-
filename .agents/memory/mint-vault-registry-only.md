...
- Custody routes (`artifacts/api-server/src/routes/custody.ts`), all atomic, with
  ...
  `INSUFFICIENT_OWNER`):
  - **Lock**: debit Growth pool, credit owner by `valuation/GRAVITY_RATE`. Logs `CUSTODY_ISSUE`. **Founder/admin-only (`isFounder` gate) — MUST stay gated; it pays out the Growth pool, so session-only would let any user mint themselves Gravity. Never revert to `requireSession`-only.**
