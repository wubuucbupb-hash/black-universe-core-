...
- **System Vault `000000000001`** (`VAULT_ACCOUNT`) — still the ONLY mint backing.
  The backing grows from THREE sources and only ever goes UP: (1) admin **approve** + **deposit** (new asset submissions), (2) **revalue-up** delta (see above), and (3) **transaction revenue** — every Foundation fee (P2P 1%, escrow 1%) buffered via `recordPoolFee(FOUNDER_ACCOUNT,…)` ALSO credits `VAULT_ACCOUNT` by the same total at `flushPendingFees` time (gated `poolAccount === FOUNDER_ACCOUNT`, logs a `DEPOSIT` "REVENUE → VAULT" tx). The Foundation still KEEPS the fee as income — the vault credit is a parallel counted increment, NOT a redirect. **Mint** (`POST /api/matrix/mint`) is a pure DRAW against the System Vault, gated 200% (`vaultGravity >= VAULT_BACKING_RATIO * coreGravity`). The vault is never decreased by any normal flow; never point custody at `VAULT_ACCOUNT`. NOTE: mint's own 1% `founderCut` is mint distribution, NOT transaction revenue — it does NOT credit the vault.
